...
import { Download, Upload, Clock, CheckCircle, XCircle, AlertTriangle } from "lucide-react";
...
export default function Faturas() {
  ...
  // Extrair dados das faturas com verificação de tipo
  const faturasData: Fatura[] = faturasResponse?.data || [];
  ...
  const faturasFiltradas = faturasData.filter((fatura: Fatura) => {
    ...
    const matchBusca = !busca || 
      ...
      (fatura.cnpj && fatura.cnpj.includes(busca));
                ...
                <TableRow key={fatura.id}>
                  <TableCell>
                    <div>
                      ...
                      {fatura.cnpj && (
                        <div className="text-sm text-muted-foreground">{fatura.cnpj}</div>
                      )}
                      {fatura.unidade && (
                        <div className="text-sm text-muted-foreground">{fatura.unidade}</div>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {fatura.valor_fatura ? `R$ ${fatura.valor_fatura.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '-'}
                  </TableCell>
                  <TableCell>
