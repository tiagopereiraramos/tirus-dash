...
import type { Fatura, Operadora, ApiResponse } from "@/types/api";

interface AprovacoesResponse {
  success: boolean;
  data: Fatura[];
}
...
export default function Aprovacoes() {
  ...
  const { data: aprovacoesResponse, isLoading: loadingAprovacoes } = useQuery<AprovacoesResponse>({ 
  ...
  // Extrair dados das aprovações
  const aprovacoesData: Fatura[] = aprovacoesResponse?.data || [];

  const aprovacoesFiltradas = aprovacoesData.filter((aprovacao: Fatura) => {
    const matchOperadora = !filtroOperadora || aprovacao.operadora_nome === filtroOperadora;
    
    let matchStatus = true;
    if (filtroStatus === "pendente") {
      matchStatus = aprovacao.status_processo === "PENDENTE_APROVACAO";
    } else if (filtroStatus === "aprovado") {
      matchStatus = aprovacao.status_processo === "APROVADA";
    } else if (filtroStatus === "rejeitado") {
      matchStatus = aprovacao.status_processo === "REJEITADA";
    }
    
    const matchBusca = !busca || 
      aprovacao.nome_sat.toLowerCase().includes(busca.toLowerCase()) ||
      (aprovacao.cnpj && aprovacao.cnpj.includes(busca));
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">
                  {aprovacoesData.filter((a: Fatura) => a.status_processo === "PENDENTE_APROVACAO").length}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">
                  {aprovacoesData.filter((a: Fatura) => a.status_processo === "APROVADA").length}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">
                  {aprovacoesData.filter((a: Fatura) => a.status_processo === "REJEITADA").length}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div>
              ...
              <Select value={filtroOperadora} onValueChange={setFiltroOperadora}>
                ...
                <SelectContent>
                  ...
                  {(operadoras || []).map((operadora: Operadora) => (
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {aprovacoesFiltradas.map((aprovacao: Fatura) => (
                <TableRow key={aprovacao.id}>
                  <TableCell>
                    <div>
                      <div className="font-medium">{aprovacao.nome_sat}</div>
                      {aprovacao.cnpj && (
                        <div className="text-sm text-muted-foreground">{aprovacao.cnpj}</div>
                      )}
                      {aprovacao.unidade && (
                        <div className="text-sm text-muted-foreground">{aprovacao.unidade}</div>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {aprovacao.valor_fatura ? `R$ ${aprovacao.valor_fatura.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '-'}
                  </TableCell>
                  <TableCell>
